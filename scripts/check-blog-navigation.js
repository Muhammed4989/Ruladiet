...
for(const category of categories){
 for(const topic of [null,...category.topics]){
  ...
  const introduction=(topic?intros.topics[topic.id]:intros.categories[category.id]);
  assert(introduction?.length,'Missing unique introduction '+route);
  for(const paragraph of introduction)assert(html.includes(paragraph),'Introduction must be in initial HTML '+route);
