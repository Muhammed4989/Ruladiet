// The 17 source fragments are edited in content/blog; this script maintains the public pages.
...
for(const p of posts){
 const file='blog/'+p.slug+'.html';let html=fs.readFileSync(root+'/'+file,'utf8');
 ...
 const hero=`<figure class="article-hero">${image(p,true)}${medical?'<figcaption>صورة توضيحية؛ لا تمثّل منتجاً طبياً بعينه أو نتيجة فحص.</figcaption>':''}</figure>`;
 ...
 const aside=`<aside class="post-sidebar"><div class="sidebar-widget sidebar-author"><a href="/رولا-علوش"><div class="sidebar-author-img"><img src="/images/rulamain.webp" alt="اختصاصية التغذية رولا علوش" width="130" height="130" loading="lazy"></div><h3>رولا علوش</h3></a><p>اختصاصية التغذية والحمية العلاجية.</p><a href="/رولا-علوش">المؤهلات والسيرة الذاتية ←</a></div><nav class="sidebar-widget sidebar-toc" aria-label="محتويات المقال"><h2>في هذه المقالة</h2>${toc}</nav></aside>`;
 ...
 const mr=elementRange(html,'post-meta');html=html.slice(0,mr.contentEnd)+`<span class="post-updated">آخر تحديث: <time datetime="${updated}">9 سبتمبر 2026</time></span>`+html.slice(mr.contentEnd);
 ...
 for(const [key,value] of Object.entries({'og:title':p.title,'og:description':p.summary,'og:image':origin+'/images/blog/'+p.key+'-1280.webp','og:image:alt':descriptions[p.key],'article:modified_time':updated,'og:image:width':'1280','og:image:height':'720'}))html=meta(html,key,value,true);
 html=html.replace(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g,(all,json)=>{const data=JSON.parse(json);if(data['@type']==='BlogPosting'){Object.assign(data,{headline:p.title,description:p.summary,image:origin+'/images/blog/'+p.key+'-1280.webp',dateModified:updated,wordCount:text(body).split(/\s+/).length,citation:p.sources.map(s=>s.url)});data.author={...data.author,url:origin+'/رولا-علوش'};delete data.about;}else if(data['@type']==='BreadcrumbList'){data.itemListElement[data.itemListElement.length-1].name=p.title;}return '<script type="application/ld+json">'+JSON.stringify(data)+'</script>';});
...
map=map.replace(/<url>[\s\S]*?<\/url>/g,entry=>{if(!entry.includes('/blog/'))return entry;return /<lastmod>/.test(entry)?entry.replace(/<lastmod>[^<]*<\/lastmod>/,`<lastmod>${updated}</lastmod>`):entry.replace('</url>',`<lastmod>${updated}</lastmod></url>`);});write('sitemap.xml',map);
console.log('Enriched 17 articles, unique covers and diagrams, related links, CTAs, metadata and sitemap.');
