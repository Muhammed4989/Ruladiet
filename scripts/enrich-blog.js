...
for(const p of posts){
 ...
 const anchored=addHeadingAnchors(body);body=anchored.body;
 const toc=`<ol class="toc-list">${anchored.headings.filter(h=>h.tag==='h2').map(h=>`<li><a href="#${h.id}">${e(h.label)}</a></li>`).join('')}</ol>`;
 ...
 const crumbs=trail(category,topic,p);
 html=html.replace(/<span class="post-category">[\s\S]*?<\/span>/,`<span class="post-category"><a href="${topicPath(category,topic)}">${e(topic.name)}</a></span>`);
 ...
 html=html.replace(/<nav class="breadcrumb\b[^>]*>[\s\S]*?<\/nav>/,breadcrumb(crumbs));
