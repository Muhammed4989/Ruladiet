...
const {categories,categoryPath,topicPath,trail,breadcrumb,breadcrumbSchema}=require('./blog-taxonomy');
...
function buildArchives(){
 const posts=require('./blog-catalog').map(p=>{
  const html=fs.readFileSync(root+'/blog/'+p.slug+'.html','utf8');
 ...
 for(const page of pages){
  ...
  const main=`<main class="blog-archive"><section class="page-hero"><div class="container">${breadcrumb(crumbs)}<div class="blog-header"><h1>${e(page.title)}</h1><p>${e(page.description)}</p></div></div></section><div class="blog-page"><div class="container">${intro}<div class="blog-layout"><div class="blog-main">${listing}</div>${sidebar(category,topic)}</div></div></div></main>`;
